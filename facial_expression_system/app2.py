...
"""
def show_input_label(text, font=("Arial", 24), x=0, y=0, width=None, height=None, wraplength=None):
    global input_label
    if input_label is not None:
        input_label.place_forget()  # 以前のラベルを削除

    input_label = tk.Label(root, text=text, font=font, wraplength=wraplength)
...
"""def show_cancel_decide_screen():
    show_input_label("この写真を選択する場合は「決定」の←キー、選択しない場合は「キャンセル」のescキーを押してください", font=("Arial", 24), x=300, y=50, width=700, height=50)
...
def show_cancel_decide_screen():
    show_input_label(
        "この写真を選択する場合は「決定」:←キー、選択しない場合は「キャンセル」:escキーを押してください", 
        font=("Arial", 19), 
        x=-860, 
        y=0, 
        width=3000, 
        height=100, 
        wraplength=3000  # 折り返しを設定
    )
    open_button.place_forget()
    cancel_button.place(x=300, y=620)
    decide_button.place(x=700, y=620)
...
result_text = tk.Text(root, height=16, width=35, font=("Arial", 20)) #文字の大きさ変える？？
